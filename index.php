...
// prepare and bind
$stmt = $conn->prepare(
    "SELECT * FROM innovation_cards  WHERE LENGTH(description) BETWEEN  140 and 160 ORDER BY RAND() LIMIT 4"
);
...
?>
  ...
  <div class="py-1" style="">
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="masonry">
            ...
            <?php while ($fetchedArray = $result->fetch_assoc()) {
                echo <<<card
    <div class="item">
    ...
    <div class="footer">
      <button class="padding:6px; width:auto;"><a href="../openai/index.php?id=${fetchedArray["id"]}" data-lightbox" > Know more <i class="fa fa-external-link" aria-hidden="true"></i></a>
        </button>
    </div>
  </div>
card;
...
<?php
$stmt = $conn->prepare(
    "SELECT * FROM innovation_cards where NOT category='Artificial Intelligence' LIMIT 4 "
);
// $stmt->bind_param("s", $userEmailToVerify);
$stmt->execute();
//fetch the results
...
<?php
$stmt = $conn->prepare(
    "SELECT * FROM innovation_cards where category='Artificial Intelligence' LIMIT 4 "
);
// $stmt->bind_param("s", $userEmailToVerify);
$stmt->execute();
//fetch the results
